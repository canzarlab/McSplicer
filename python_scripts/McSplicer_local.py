...
def get_DE_gene_list(DE_gene_file):

    gene_list = []

    with open(DE_gene_file,'rb') as in_file:
        for line in in_file:
            if line != '':
                line = line #[:-2] #remove \n
                gene_id = line.split()[0]
...
if __name__ == "__main__":
    ...
    gene_l_file = '/data/israa/mcsplicer_paper_genome_simulation/SRR6987574_genes.txt'
    gene_list = get_DE_gene_list(gene_l_file)
    #print('>>>>len gene list=',len(gene_list))
    #print(gene_list)
    ...
    for gene_id in gene_list:
        ...
        if gene_id not in all_gene_dict: continue
        ...
        if len(start_sites_dict) > 0:
            ...
            out_f = write_output(gene_id, start_sites_dict, end_sites_dict, p_arr_list, q_arr_list, out_dir, no_steps,out_file,add_start_stop_nodes,chr_id,strand)

            print('\tOutput is written to the folder',out_f)
        else:
